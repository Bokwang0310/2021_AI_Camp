...
# 각 감정에 대한 점수
emotion_score = {"Angry": 5, "Disgusting": 0, "Fearful": 5,
                 "Happy": 5, "Sad": 5, "Neutral": 0, "Surprising": 5}
...
def make_result_text(inner_result_text, max_emotion_text, movie_score):
    ...
    for ii, line in enumerate(inner_result_text.split("\n")):
        y = y0 + ii*dy
        cv2.putText(result_image, line, (50, y),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 1, cv2.LINE_AA)

    cv2.putText(result_image, max_emotion_text, (50, 230),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 1, cv2.LINE_AA)
    cv2.putText(result_image, movie_score, (50, 260),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 1, cv2.LINE_AA)
...
make_result_text(
    result_text, f"Max Emotion: {most_emotion}", f"Score: {round(score / count, 1)}")
